...
if [ -n "$libPath" ]; then
	...
	## SFML extlibs
	cp "$libPath/libsndfile-1.dll" "$prefix"
	cp "$libPath/openal32.dll" "$prefix"
	## SFML
	cp "$libPath/sfml-audio.dll" "$prefix"
	cp "$libPath/sfml-graphics.dll" "$prefix"
	cp "$libPath/sfml-system.dll" "$prefix"
	cp "$libPath/sfml-window.dll" "$prefix"
	## lib gcc
