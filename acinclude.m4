...
[
	...
	if test "$IS_LINUX" = 'true'; then
		AM_CHECK_LIB_HEADER([GL], [gl], [GL/gl.h], [GL], [], AC_MSG_ERROR([	/!\ This programme requires OpenGL !]))
	elif test "$IS_MINGW" = 'true'; then
		AM_CHECK_LIB_HEADER([GL], [gl], [GL/gl.h], [opengl32 glu32], [], [AC_MSG_ERROR([	/!\ This programme requires OpenGL (opengl32 glu32) !])])
	elif test "$IS_MAC" = 'true'; then
		CHECK_FRAMEWORK_LIST([OGLMAC], [OpenGL GLUT])
	fi
])
